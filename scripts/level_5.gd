...
func _on_number_timer_timeout():
	if numbers.size() < max_numbers + additional_numbers and not level_completed:
		...
		if randi() % 2 == 0:
			 # Seleccionar un número aleatorio entre 0-9
			number.text = str(randi() % 10)
		else:
			# Generar una letra aleatoria entre A-Z usando valores ASCII
			var letter = String.chr(randi() % 26 + 65)  # 65 es el valor ASCII de 'A'
			number.text = letter
