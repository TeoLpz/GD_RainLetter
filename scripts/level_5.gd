...
func _ready():
	...
	completion_label.text = "Nivel 5 Completado"
...
func _on_letter_timer_timeout():
	if letters.size() < max_letters + additional_letters and not level_completed:
		# Generar letras o números dentro de un rango definido
		var min_x = 130
		var max_x = get_viewport().size.x - 130
		var start_y = -50

		# Lista de caracteres válidos (A-Z y 1-9)
		var valid_characters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ123456789"

		# Crear una nueva letra o número aleatorio
		var letter = FallingLetter.new()
		letter.text = valid_characters[randi() % valid_characters.length()]
		letter.position = Vector2(randf_range(min_x, max_x), start_y)
		letter.speed = randf_range(fall_speed_range.x, fall_speed_range.y)
		add_child(letter)

		# Agregar a la lista de letras en pantalla
